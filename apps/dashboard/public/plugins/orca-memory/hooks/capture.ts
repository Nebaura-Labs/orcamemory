...
function sanitizeCapturedText(text: string): string {
	return text
		.replace(/<orca-memory-context>[\s\S]*?<\/orca-memory-context>\s*/g, "")
		.trim();
}
...
export function buildCaptureHandler(
	...
	return async (event: Record<string, unknown>) => {
		if (
			!(event.success && Array.isArray(event.messages)) ||
		...
		log.info(
			`[orca-memory] capture: ${captured.length} texts (${content.length} chars)`
		);
		log.info(
			`[orca-memory] model=${model}, tokens: prompt=${tokensPrompt}, completion=${tokensCompletion}, total=${tokensTotal}`
		);
		if (discordMeta.guildName) {
			log.info(
				`[orca-memory] discord: guild=${discordMeta.guildName}, user=${discordMeta.displayName} (${discordMeta.userId})`
			);
		}
