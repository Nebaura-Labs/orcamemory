...
const parseAuthPayload = async (
	...
	if (!(payload.keyId && payload.secret)) {
		return {
			response: new Response("Missing keyId or secret", { status: 400 }),
		};
	}
...
export const issueKey = mutation({
	...
	handler: async (ctx, args) => {
		...
		// If rotating a specific agent, verify it exists and belongs to this project
		if (args.agentId) {
			const existingAgent = await ctx.db.get(args.agentId);
			if (!existingAgent || existingAgent.projectId !== args.projectId) {
		...
		// Use provided agentId for rotation, or first existing agent, or null for new
		let agentId = args.agentId ?? existingAgents[0]?._id ?? null;
		if (!agentId) {
		...
		const activeKey = await ctx.db
			.query("agentKeys")
			.withIndex("agentId", (queryBuilder) =>
				queryBuilder.eq("agentId", agentId)
			)
			.filter((queryBuilder) =>
				queryBuilder.eq(queryBuilder.field("revokedAt"), undefined)
			)
			.first();
...
export const getActiveKeyByProject = query({
	...
	handler: async (ctx, args) => {
		...
		const key = await ctx.db
			.query("agentKeys")
			.withIndex("agentId", (queryBuilder) =>
				queryBuilder.eq("agentId", agent._id)
			)
			.filter((queryBuilder) =>
				queryBuilder.eq(queryBuilder.field("revokedAt"), undefined)
			)
...
export const getKeyRecord = internalQuery({
	...
	handler: async (ctx, args) => {
		return await ctx.db
			.query("agentKeys")
			.withIndex("keyId", (queryBuilder) =>
				queryBuilder.eq("keyId", args.keyId)
			)
			.first();
	},
});
