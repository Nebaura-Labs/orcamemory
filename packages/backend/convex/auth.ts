...
const sendOtpEmail = async (
    ...
    <div style="background:#000;font-family:Arial,Helvetica,sans-serif;padding:40px">
      <div style="max-width:600px;margin:0 auto;padding:0 40px;color:#e5e5e5">
        ...
        <div style="background:#000;border:1px solid #333;border-radius:8px;padding:24px;text-align:center;margin-bottom:24px">
...
function createAuth(ctx: GenericCtx<DataModel>) {
	const trustedOrigins = [siteUrl, frontendUrl, ...frontendUrls].filter(
		Boolean
	);
	return betterAuth({
		baseURL: siteUrl,
		...
		plugins: [
			...
			emailOTP({
				...
				async sendVerificationOTP({ email, otp, type }) {
					if (process.env.NODE_ENV !== "production") {
						console.log(
							`[OTP] sendVerificationOTP email=${email} type=${type}`
						);
					}
